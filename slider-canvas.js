//  This is a copy of slider-canvas.js, modified and fudged to work with CA.html
...
SliderCanvas.prototype.setMinMax = function (sliderNum, min, max) {
  this.sliders[sliderNum].min = min;
  this.sliders[sliderNum].max = max;
  this.setValue(sliderNum, this.value(sliderNum));
};
SliderCanvas.prototype.setStep = function (sliderNum, step) {
  this.sliders[sliderNum].step = step;
  this.setValue(sliderNum, this.value(sliderNum));
};
...
SliderInfo.prototype.hitBar = function (x, y) {
  return (
    y > this.top + 30 && y < this.top + 40 && x > 15 && x < 15 + this.length
  );
};
SliderInfo.prototype.setBar = function (x) {
  // returns true/false to say whether value changed
...
SliderInfo.prototype.dragTab = function (newTabLeft) {
  // returns true/false to say whether value changed
  var actualTL;
  if (newTabLeft < 5) {
    actualTL = 5;
...
SliderCanvas.prototype.onChange =
  function (/* newValue, oldValue, sliderNumber */) {};
SliderCanvas.prototype.onDragStart = null;
SliderCanvas.prototype.onDragEnd = null;
...
SliderCanvas.prototype.draw = function () {
  ...
  // note: turns out that on resize of the canvas, the graphics context lost its font
  if (
    this.rightMargin == null ||
    r.width != this.canvasWidth ||
    r.height != this.canvasHeight
  ) {
    // signal to compute right margin and slider lengths
    ...
    for (i = 0; i < this.sliders.length; i++) {
      ...
      if (slider.label) {
        margin = Math.max(
          margin,
          this.graphics.measureText(slider.label + " =").width
        );
      }
    }
    margin = margin == 0 ? 10 : margin + 20;
    var length = r.width - margin - 15;
    this.rightMargin = margin;
    var spacing = r.height / (this.sliders.length + 1);
    for (i = 0; i < this.sliders.length; i++) {
    ...
    if (this.sliders.length == 1) {
      this.sliders[0].top = -6; // fudging for the lambda slider
    }
  }
